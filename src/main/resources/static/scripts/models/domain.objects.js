...
qat.model.fnTelefones = function(_telefone, modelAction, _user, $log) {
    ...
    if (_telefone) {
        ...
        if (localStorage.getItem('empresa') == null ||
            ...
            _entidadeId = JSON.parse(localStorage.getItem('empresa')).id;
        }
...
qat.model.fnDocumento = function(_documento, _modelAction, _userId, $log) {
    ...
    estoque = {
     ...
     //   estado: _documento.estado,
        documentoTypeEnumValue: _documento.documentoTypeEnumValue ? _documento.documentoTypeEnumValue : 0,
        numero: _documento.numero,
        parentId: 0,
        emprId: _emprId ? _emprId : 0,
        processId: 0,
        tableEnumValue: 0,
        modelAction: _modelAction,
        createUser: _userId,
...
qat.model.fnUsuario = function(_oObjet, modelAction, user, $log) {
    ...
    this.senha = _oObjet.senha;
    this.nome = _oObjet.nome;
    this.cpf = _oObjet.cpf ? _oObjet.cpf.numero : "00";
...
qat.model.Marca = function(_oObjet, _modelAction, _user) {
    ...
    this.emailList = [];
    _oObjet.emailList ? this.emailList.push(qat.model.fnEmails(_oObjet.emailList ? _oObjet.emailList[0] : {}, (_oObjet.emailList[0] && _oObjet.emailList[0].id) ? _modelAction : 'INSERT', _user)) : null;
    this.enderecoList = [];
    _oObjet.enderecoList ? this.enderecoList.push(qat.model.fnEndereco(_oObjet.enderecoList ? _oObjet.enderecoList[0] : {}, (_oObjet.enderecoList && _oObjet.emailList[0].id) ? _modelAction : 'INSERT', _user)) : null;
    this.telefoneList = [];
    _oObjet.telefoneList ? this.telefoneList.push(qat.model.fnTelefones(_oObjet.telefoneList ? _oObjet.telefoneList[0] : {}, (_oObjet.telefoneList[0] && _oObjet.telefoneList[0].id) ? _modelAction : 'INSERT', _user)) : null;
    this.parentId = _oObjet.parentId;
    this.emprId = JSON.parse(localStorage.getItem('empresa')).id;
    this.processId = _oObjet.processId;
    this.tableEnumValue = _oObjet.tableEnumValue;
...
qat.model.Empresa = function(_oObjet, _modelAction, _user, $log) {
    ...
    if (_oObjet != undefined) {
        ...
        this.socios = _oObjet.socios ? _oObjet.socios : null;
        this.tipo = _oObjet.tipo;
        this.permissaoTypeEnum = _oObjet.permissaoTypeEnum ? _oObjet.permissaoTypeEnum : 0;
        this.numFunc = _oObjet.numFunc;
        this.tipoPessoa = _oObjet.tipoPessoa;
        this.cpfResponsavel = _oObjet.cpfResponsavel;
        this.primeiroAcesso = _oObjet.primeiroAcesso ? _oObjet.primeiroAcesso : 0;
        ...
        this.entidadeEnumValue = _oObjet.entidadeEnumValue;
        this.regime = _oObjet.regime ? qat.model.fnRegime(_oObjet.regime, _oObjet.regime.id ? _modelAction : "INSERT", _user, $log) : null;
        this.cpfCnpj = _oObjet.cpfCnpj ? _oObjet.cpfCnpj : null;
        this.enderecos = _oObjet.enderecos ? _oObjet.enderecos : null;
        this.emails = _oObjet.emails,
            this.telefones = _oObjet.telefones,
            this.cnaes = _oObjet.cnaes ? qat.model.Cnaes(_oObjet.cnaes, _modelAction, _user, $log) : null;
...
qat.model.fnNFNotaInfoTotal = function(_NFNotaInfoTotal, modelAction, _user) {
    ...
    NFNotaInfoTotal = {
        id: _id,
        icmsTotal: _NFNotaInfoTotal.icmsTotal ? qat.model.fnNFNotaInfoICMSTotal(_NFNotaInfoTotal.icmsTotal, modelAction, _user) : null,
        issqnTotal: _NFNotaInfoTotal.issqnTotal ? qat.model.fnNFNotaInfoISSQNTotal(_NFNotaInfoTotal.issqnTotal, modelAction, _user) : null,
        retencoesTributos: _NFNotaInfoTotal.retencoesTributos ? qat.model.fnNFNotaInfoRetencoesTributos(_NFNotaInfoTotal.retencoesTributos, modelAction, _user) : null,
        parentId: 0,
        emprId: _emprId,
        processId: 0,
        tableEnumValue: 0,
