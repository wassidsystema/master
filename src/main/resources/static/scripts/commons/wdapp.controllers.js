	...
	commonControllers.controller('NavController', ['$element','$compile','$http','$location','$rootScope','$scope', 'TaskStorage', 'filterFilter','localStorageService','SysMgmtData', function($element,$compile,$http,$location, $rootScope, $scope, TaskStorage, filterFilter,localStorageService,SysMgmtData) {
      ...
      $scope.returnUrl = function(oMenu,oPagina) {
    	//  debugger
	      if(oMenu.nivel == 0)
	      {
	    	  if(oMenu && oMenu.groupMenu.length > 0 && oMenu.groupMenu[0].paginasss.length > 0 && oMenu.groupMenu[0].paginasss[0].existe == 1){
	    		  return oMenu.groupMenu[0].paginasss[0].paginaFisica;
	    	  } else if(oMenu && oMenu.groupMenu.length > 0 && oMenu.groupMenu[0].paginasss.length > 0){
	    		  return "#/ajax/main_form?id="+oMenu.groupMenu[0].paginasss[0].id;
	    	  } else {
	    		  return "#/ajax/main_form?id=1";
	    	  }
	      } else {
...
//  	});
      ...
      $scope.pageName = function(oMenu) {
	      ...
	      if(oMenu.nivel == 0)
	      {
	    	  return oMenu.groupMenu.length > 0 ? oMenu.groupMenu[0].paginasss.length > 0 ? oMenu.groupMenu[0].paginasss[0].pagina : oMenu.groupMenu[0].nome : "Sem Nome";
	      } else {
	    	  return oMenu.groupMenu.length > 0 ? oMenu.groupMenu[0].nome : "Sem Nome(Erro Dicionario)";
	      }
      };
      ...
      $scope.pageIcon = function(oMenu) {
      	//  debugger
  	      if(oMenu.nivel == 0)
  	      {
  	    	  return oMenu.groupMenu.length > 0 ? oMenu.groupMenu[0].icone : oMenu.icon;
  	      } else {
  	    	  return oMenu.icon;
  	      }
        };
