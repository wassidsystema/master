(function() {
    angular.module('wdApp.apps.buscarQuadra', []).controller('BuscaQuadraController', ['$scope', 'jogoFactory', 'AuthService', '$rootScope', '$location', '$http', '$interval', 'SysMgmtData', 'toastr', 'toastrConfig', '$uibModal',
        function($scope, jogoFactory, AuthService, $rootScope, $location, $http, $interval, SysMgmtData, toastr, toastrConfig, $uibModal) {
            ...
            AuthService.fetchAllEmpresa(vm.empresa, function(response) {
                ...
                $scope.avaliacao = response[0].avaliacao;
                $scope.loading = false;
            ...
            $scope.calcMediaAvaliacao = function(listAvaliacao, listOpcao) {
                let total = 0;
                if (listAvaliacao) {
                    let avaliacaoItens = listAvaliacao.avaliacaoItens;
                    for (let y = 0; y < avaliacaoItens.length; y++) {
                        total = total + avaliacaoItens[y].nota;
                    }

                }

                return total / (listOpcao.length > 0 ? listOpcao.length : 1);
            }
